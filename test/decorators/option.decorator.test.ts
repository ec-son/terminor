...
describe("Option decorator", () => {
  ...
  beforeEach(() => {
    mockMetadata = {
      ...
      options: [],
      subCommandNames: [],
      handlers: [{ methodKey: "handler", on: "handler", parameters: [], isFirstHandler: true }],
      unknownOptions: [],
      excessArguments: [],
      help: { flag: "--help", disabled: true },
    } as any;

    (commandContainer as unknown as { getCommand: jest.Mock }).getCommand = jest.fn((inst: any) => {
  ...
  it("should add date description suffix", () => {
    class TestCmd {
      @((Option as any)({ flag: "date", type: "date", description: "Select a date" }))
      date!: Date;
    ...
    expect(mockMetadata.options[0].description).toContain("YYYY-MM-DD");
  });
});
